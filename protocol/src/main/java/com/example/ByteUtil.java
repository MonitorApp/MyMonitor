...
public class ByteUtil
{
    ...
    {
    	return (0xff & bytes[offset]) | (0xff00 & (bytes[offset+1] << 8)) 
    ...
    public static String byte2HexString(byte[] bytes, int offset, int len)
    {
    	return byte2HexStr(SubBytes(bytes, offset, len));
    }
    
	static public byte[] SubBytes(byte[] srcBytes, int pos, int len)
	{
		byte[] byteRet = new byte[len];
		System.arraycopy( srcBytes, pos, byteRet, 0, len );	
		return byteRet;
	}
}
