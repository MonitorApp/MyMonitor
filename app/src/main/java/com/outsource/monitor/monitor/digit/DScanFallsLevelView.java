 ...
 */
public class DScanFallsLevelView extends BaseTextureView implements DigitDataReceiver {
    ...
    private AtomicReference<DScanParser48278.DataHead> mHead = new AtomicReference<>();

    public DScanFallsLevelView(Context context) {
        super(context);
        init();
    }

    public DScanFallsLevelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void onReceiveDigitData(DScanParser48278.DataValue dsData) {
        ...
        updateFallLevels(new FallRow(System.currentTimeMillis(), dsData.valueList));
    }

    @Override
    public void onReceiveDigitHead(DScanParser48278.DataHead dsHead) {
        if (dsHead == null || dsHead.pointCount <= 0) {
            LogUtils.d("数字扫描帧头为空！");
            return;
        }
        mHead.set(dsHead);
        startFreqency = DisplayUtils.toDisplayFrequency(dsHead.nStartFreq);
        endFrequency = DisplayUtils.toDisplayFrequency(dsHead.endFreq);
    }
}
