...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    private ColorBuffer[] mColorBuffers;
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        buffer.setBarWidth(mChart.getHorizontalBarData().getBarWidth());

        buffer.feed(dataSet);

        trans.pointValuesToPixel(buffer.buffer);

        ColorBuffer colorBuffer = mColorBuffers[index];
        colorBuffer.feed(dataSet);
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            int[] colors = colorBuffer.colors[j / 4];
            if (colors.length >= 2) {
                LinearGradient rectShader = new LinearGradient(buffer.buffer[j], 0, buffer.buffer[j + 2], 0, colors, null, Shader.TileMode.MIRROR);
                mRenderPaint.setShader(rectShader);
            }
    ...
    public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            List<IBarDataSet> dataSets = mChart.getHorizontalBarData().getDataSets();
            ...
            for (int i = 0; i < mChart.getHorizontalBarData().getDataSetCount(); i++) {
